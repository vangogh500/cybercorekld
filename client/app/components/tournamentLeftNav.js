        ...
        <div className="panel-heading text-center z-depth-2"><h4>Navigate</h4></div>
        {
          (this.state.location == '') ? (
            <ul className="nav nav-pills nav-stacked" role="tablist">
              <li className={"nav-item " + (this.state.active == '' ? 'active z-depth-4' : 'z-depth-2')}>
                  <a className="nav-link" data-toggle="tab" role="tab">Tournaments</a>
              </li>
              <li className={"nav-item " + (this.state.active == '/teams' ? 'active z-depth-4' : 'z-depth-2')} onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseExit}>
                <Link to="/auth/tournaments/teams" className="nav-link" data-toggle="tab" role="tab">Teams</Link>
              </li>
            </ul>
          ) : (
            <ul className="nav nav-pills nav-stacked" role="tablist">
              <li className={"nav-item " + (this.state.active == '' ? 'active z-depth-4' : 'z-depth-2')} onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseExit}>
                  <Link to="/auth/tournaments" className="nav-link" data-toggle="tab" role="tab">Tournaments</Link>
              </li>
              <li className={"nav-item " + (this.state.active == '/teams' ? 'active z-depth-4' : 'z-depth-2')}>
